...
struct TBDGTapLocation {
  ...
  func screenToWorldOnZPlane(
    ...
    camera: VRTMCameraPerspective
  ) -> F3? {

    // Convert to NDC
    let ndc = screenToNdc(
      screenWidth: screenDimensions.pointWidth,
      screenHeight: screenDimensions.pointHeight
    )
    ...
    let rayDirection = normalize(
      F3(rayPointInWorld.x, rayPointInWorld.y, rayPointInWorld.z) - rayOrigin
    )
